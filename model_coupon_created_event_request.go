/*
Talon.One API

Use the Talon.One API to integrate with your application and to manage applications and campaigns:  - Use the operations in the [Integration API section](#integration-api) are used to integrate with our platform - Use the operation in the [Management API section](#management-api) to manage applications and campaigns.  ## Determining the base URL of the endpoints  The API is available at the same hostname as your Campaign Manager deployment. For example, if you access the Campaign Manager at `https://yourbaseurl.talon.one/`, the URL for the [updateCustomerSessionV2](https://docs.talon.one/integration-api#operation/updateCustomerSessionV2) endpoint is `https://yourbaseurl.talon.one/v2/customer_sessions/{Id}`

API version:
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package talon

import (
	"bytes"
	"encoding/json"
	"fmt"
	"time"
)

// checks if the CouponCreatedEventRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CouponCreatedEventRequest{}

// CouponCreatedEventRequest struct for CouponCreatedEventRequest
type CouponCreatedEventRequest struct {
	TotalResultSize int64 `json:"TotalResultSize"`
	// Timestamp when the batch was created.
	BatchedAt *time.Time    `json:"BatchedAt,omitempty"`
	EventType string        `json:"EventType"`
	Data      []interface{} `json:"Data"`
}

type _CouponCreatedEventRequest CouponCreatedEventRequest

// NewCouponCreatedEventRequest instantiates a new CouponCreatedEventRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func BuildCouponCreatedEventRequest(totalResultSize int64, eventType string, data []interface{}) *CouponCreatedEventRequest {
	this := CouponCreatedEventRequest{}
	this.TotalResultSize = totalResultSize
	this.EventType = eventType
	this.Data = data
	return &this
}

// NewCouponCreatedEventRequestWithDefaults instantiates a new CouponCreatedEventRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCouponCreatedEventRequestWithDefaults() *CouponCreatedEventRequest {
	this := CouponCreatedEventRequest{}
	return &this
}

// GetTotalResultSize returns the TotalResultSize field value
func (o *CouponCreatedEventRequest) GetTotalResultSize() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.TotalResultSize
}

// GetTotalResultSizeOk returns a tuple with the TotalResultSize field value
// and a boolean to check if the value has been set.
func (o *CouponCreatedEventRequest) GetTotalResultSizeOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TotalResultSize, true
}

// SetTotalResultSize sets field value
func (o *CouponCreatedEventRequest) SetTotalResultSize(v int64) {
	o.TotalResultSize = v
}

// GetBatchedAt returns the BatchedAt field value if set, zero value otherwise.
func (o *CouponCreatedEventRequest) GetBatchedAt() time.Time {
	if o == nil || IsNil(o.BatchedAt) {
		var ret time.Time
		return ret
	}
	return *o.BatchedAt
}

// GetBatchedAtOk returns a tuple with the BatchedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CouponCreatedEventRequest) GetBatchedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.BatchedAt) {
		return nil, false
	}
	return o.BatchedAt, true
}

// HasBatchedAt returns a boolean if a field has been set.
func (o *CouponCreatedEventRequest) HasBatchedAt() bool {
	if o != nil && !IsNil(o.BatchedAt) {
		return true
	}

	return false
}

// SetBatchedAt gets a reference to the given time.Time and assigns it to the BatchedAt field.
func (o *CouponCreatedEventRequest) SetBatchedAt(v time.Time) {
	o.BatchedAt = &v
}

// GetEventType returns the EventType field value
func (o *CouponCreatedEventRequest) GetEventType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.EventType
}

// GetEventTypeOk returns a tuple with the EventType field value
// and a boolean to check if the value has been set.
func (o *CouponCreatedEventRequest) GetEventTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EventType, true
}

// SetEventType sets field value
func (o *CouponCreatedEventRequest) SetEventType(v string) {
	o.EventType = v
}

// GetData returns the Data field value
func (o *CouponCreatedEventRequest) GetData() []interface{} {
	if o == nil {
		var ret []interface{}
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *CouponCreatedEventRequest) GetDataOk() ([]interface{}, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *CouponCreatedEventRequest) SetData(v []interface{}) {
	o.Data = v
}

func (o CouponCreatedEventRequest) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CouponCreatedEventRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["TotalResultSize"] = o.TotalResultSize
	if !IsNil(o.BatchedAt) {
		toSerialize["BatchedAt"] = o.BatchedAt
	}
	toSerialize["EventType"] = o.EventType
	toSerialize["Data"] = o.Data
	return toSerialize, nil
}

func (o *CouponCreatedEventRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"TotalResultSize",
		"EventType",
		"Data",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCouponCreatedEventRequest := _CouponCreatedEventRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varCouponCreatedEventRequest)

	if err != nil {
		return err
	}

	*o = CouponCreatedEventRequest(varCouponCreatedEventRequest)

	return err
}

type NullableCouponCreatedEventRequest struct {
	value *CouponCreatedEventRequest
	isSet bool
}

func (v NullableCouponCreatedEventRequest) Get() *CouponCreatedEventRequest {
	return v.value
}

func (v *NullableCouponCreatedEventRequest) Set(val *CouponCreatedEventRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableCouponCreatedEventRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableCouponCreatedEventRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func BuildNullableCouponCreatedEventRequest(val *CouponCreatedEventRequest) *NullableCouponCreatedEventRequest {
	return &NullableCouponCreatedEventRequest{value: val, isSet: true}
}

func (v NullableCouponCreatedEventRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCouponCreatedEventRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
