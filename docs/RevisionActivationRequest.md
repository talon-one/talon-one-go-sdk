...
------------ | ------------- | ------------- | -------------
**UserIds** | **[]int64** | The list of IDs of the users who will receive the activation request. | 
**ActivateAt** | Pointer to **time.Time** | Time when the revisions are finalized after the &#x60;activate_revision&#x60; operation. The current time is used when left blank.  **Note:** It must be an RFC3339 timestamp string.  | [optional] 
